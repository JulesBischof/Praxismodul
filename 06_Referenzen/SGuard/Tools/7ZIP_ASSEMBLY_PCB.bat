...
set ARCHIVE_PCB=%ARCHIVE_PCB:~1,-1%
...
rem Ordner, welcher dem ZIP-File zugefügt werden soll
set OutputFolder=%ARCHIVE_PCB%
for /F "delims=" %%a in ('dir "%ARCHIVE_PCB%" /b /o:n /a:d') do (
   set OutputFolder=%%a
)

del "%OutputFolder%.zip"
rem Cleanup
del "%ARCHIVE_PCB%\%OutputFolder%\%OutputFolder%\Status Report.Txt"
del "%ARCHIVE_PCB%\%OutputFolder%\%OutputFolder%\PCB_ASCII\__Previews\"

IF EXIST ".\%ARCHIVE_PCB%\%OutputFolder%\" (
	%PROJ_PATH%Tools\7-Zip\7z.exe a "%OutputFolder%.zip" ".\%ARCHIVE_PCB%\%OutputFolder%\*"
)

rem -----------------------------------------------------------

rmdir /S /Q "%ARCHIVE_ASSY%"
rmdir /S /Q "%ARCHIVE_PCB%"
